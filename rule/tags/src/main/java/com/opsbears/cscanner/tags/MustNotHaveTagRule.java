...
public class MustNotHaveTagRule implements TaggedResourceRule {
    public final static String MUST_NOT_HAVE_TAG = "MUST_NOT_HAVE_TAG";
    ...
    public List<RuleResult> evaluate(TaggedResourceConnection connection) {
        ...
        Stream<RuleResult> newStream = stream.map(
            item -> new RuleResult(
                ...
                item.resourceRegion,
                item.resourceName,
                item.tags.containsKey(tagName) ? RuleResult.Compliancy.NONCOMPLIANT : RuleResult.Compliancy.COMPLIANT,
                item.tags.containsKey(tagName)?
                    new ImmutableArrayList<RuleResult.Violation>()
                        .withAdd(new RuleResult.Violation(null, "Disallpwed tag " + tagName))
